...
contract PrivateSale is ReentrancyGuard, Context, Ownable {
    ...
    constructor(
        ...
        token_usdc = IERC20(0x2bF9b864cdc97b08B6D79ad4663e71B8aB65c45c); // USDC token address
        _rate = rate;
        _wallet = wallet;
        _token = token;
    }
        ...
        icoActive
    {
        ...
        for (uint i = 0; i < 8; i++) {
            _contributions[beneficiary].push(amount.div(8));
        }
        _initialTimestamp[beneficiary] = block.timestamp;
        token_usdc.transferFrom(address(beneficiary), address(this), amount);
    ...
    function _forwardFunds() internal {
        token_usdc.transfer(_wallet, token_usdc.balanceOf(address(this)));
    }
    ...
    function withdraw() external onlyOwner icoNotActive {
        ...
        require(token_usdc.balanceOf(address(this)) > 0, "Contract has no money");
        token_usdc.transfer(_wallet, token_usdc.balanceOf(address(this)));
    }
    ...
    // modifier checkApprove(uint256 amount) {
    //     require(token_usdc.approve(address(this), amount), "Error");
    //     _;
    // }
}
